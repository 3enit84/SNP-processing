...
OUTPUT = "ALL_ARDS_Alleles_Major_Minor_M0_from_All.txt"
...
while line != "":
      ...
      if Allele_lst[Minor] == 0:#no minor allele in whites controls
            ...
            for i in range(14,18,1):#disease EA alleles start at rec[14] and span 16 entries
                  Allele_lst = Allele_lst + [int(rec[i])]                  
            for i in range(22,26,1):#sum up alleles calls from all diseases in EA
                  Allele_lst[i-22] = Allele_lst[i-22] + int(rec[i])
            for i in range(30,34,1):#sum up alleles calls from all diseases in AA
                  Allele_lst[i-30] = Allele_lst[i-30] + int(rec[i])
            for i in range(38,42,1):#sum up alleles calls from all diseases in AA
                  Allele_lst[i-38] = Allele_lst[i-38] + int(rec[i])
            for i in range(46,50,1):#sum up alleles calls from all diseases in EA
                  Allele_lst[i-46] = Allele_lst[i-46] + int(rec[i])
